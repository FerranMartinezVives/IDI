...
{
  glm::mat4 TG(1.0f);
  TG = glm::rotate(TG, angle_rotacio, glm::vec3(0.0, 0.0, 1.0));
  TG = glm::scale(TG, glm::vec3(factor_escala, factor_escala, factor_escala));
...
void MyGLWidget::modelTransformCavall(glm::vec3 posicio)
{
  ...
  TG = glm::translate(TG, glm::vec3(-2.7f / 2.0f, -1.7f / 2.0f, 0.0));
  glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]);
}
...
//  glViewport (0, 0, ample, alt);
  ...
  for (int i = 0; i < 8; ++i) {
    for (int j = 0; j < 8; ++j) {
      ...
      modelTransformQuadrat(glm::vec3(-0.875 + j * 0.25, 0.875 - i * 0.25, 0.0));
      if ((i + j) % 2 == 0) glUniform3fv(colorLoc, 1, &blancTauler[0]);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  switch (event->key()) {
    case Qt::Key_S:
      if (factor_escala < 2.0) factor_escala += 0.1;
    	break;
    case Qt::Key_D:
      if (factor_escala > 0.1) factor_escala -= 0.1;
      break;
    case Qt::Key_E:
      angle_rotacio += (float)M_PI / 180.0f;
    	break;
    case Qt::Key_R:
      angle_rotacio -= (float)M_PI / 180.0f;
    	break;
